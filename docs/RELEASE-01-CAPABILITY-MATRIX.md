...
| Signature carving | jpg/png/gif/bmp/tiff/pdf/ole/zip/rar/7z/mp3/wav/avi/mp4 (+ optional exe); Quick Verify + Evidence **Run carving** | UNIT_VERIFIED on synthetic `testdata/demo` | **Not** full file recovery. Loads image into RAM — refuses above **64 MiB** (`ScopeDenied` / Limited UI). PE/`MZ` excluded from Quick Verify default set (noise). OOXML covered via ZIP magic. Streaming carve = future |
