...
impl CaseDb {
    ...
    fn apply_v3(&self) -> LabResult<()> {
        let sql = include_str!("../migrations/003_evidence_registry.sql");
        self.conn
            .execute_batch(sql)
            .map_err(|e| LabError::Internal {
                detail: format!("apply v3 migration: {e}"),
            })
    }

    fn apply_v4(&self) -> LabResult<()> {
        let sql = include_str!("../migrations/004_bookmarks.sql");
        self.conn
            .execute_batch(sql)
            .map_err(|e| LabError::Internal {
                detail: format!("apply v4 migration: {e}"),
            })
    }

    fn apply_v5(&self) -> LabResult<()> {
        let sql = include_str!("../migrations/005_validation_hooks.sql");
        self.conn
            .execute_batch(sql)
            .map_err(|e| LabError::Internal {
                detail: format!("apply v5 migration: {e}"),
            })
    }
